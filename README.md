...
* Master: read / write - является единственной точкой записи
* Slave: read-only - снижает нагрузку на Master. Подходит для отчётов и аналитики

Вертикальный шардинг реализован внутри одной базы данных путём логического разделения данных по таблицам (users, books, stores), каждая из которых представляет отдельную бизнес-сущность.
